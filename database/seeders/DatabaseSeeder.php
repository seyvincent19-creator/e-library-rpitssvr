    ...
    public function run(): void
    {
        ...
        foreach ($degrees as $d) {
            Degree::firstOrCreate(
                ['degree_level' => $d['degree_level'], 'majors' => $d['majors'], 'study_time' => $d['study_time']],
                $d
            );
        }
        ...
        foreach ($studentsData as $s) {
            Student::firstOrCreate(['student_code' => strtolower($s['student_code'])], $s);
        }
        ...
        foreach ($lecturersData as $l) {
            Lecturer::firstOrCreate(['lecturer_code' => strtolower($l['lecturer_code'])], $l);
        }
        ...
        User::firstOrCreate(['email' => 'librarian@example.com'], [
            'name' => 'Librarian', 'password' => $password, 'role' => 'librarian',
        ]);
        ...
        User::firstOrCreate(['email' => 'user@example.com'], [
            'name' => 'User', 'password' => $password, 'role' => 'user',
        ]);
        ...
        // ── 6. Borrow Books (skip if already exist) ───────────────────────────────
