    ...
    <main>
        ...
        <div class="info-data">
            ...
            <div class="card-dashboard" data-aos="fade-up">
                <div class="head">
                    <h2>Books Returned Today</h2>
                    <span class="material-symbols-rounded icon icon-book">rotate_auto</span>
                </div>
                <p class="number-book">{{ $returnsToday }}</p>
            </div>
            ...
            <div class="card-dashboard" data-aos="fade-up">
                ...
                <p class="number-tol_book">{{ $totalReturns }}</p>
            </div>

        </div>

        <div class="recent-activity">
            <h5 class="activity-title">Recent Returns

                <a href="{{ route('return_book.create') }}" class="btn btn-primary float-end mb-2">
            ...
            <table id="example" class="table table-striped table-hover text-center table-bordered nowrap">
                ...
                <tbody class="table-light">
                    ...
                    @foreach ($returns as $return)
                        <tr>
                            <td>{{ $return->borrowBook?->user?->code_member ?? '-' }}</td>
                            <td>{{ $return->borrowBook?->user?->name_formatted ?? '-' }}</td>
                            <td class="text-start">{{ Str::limit($return->borrowBook?->book?->title, 30, '...') ?? '-' }}</td>
                            <td>{{ $return->borrowBook?->due_date ?? '-' }}</td>
                            ...
                            <td>
                                ...
                                @endphp
                                <span class="badge bg-{{ $badge }}">{{ Str::ucfirst($return->status) }}</span>
                            </td>
                            <td>
                                <a class="btn btn-success btn-sm me-1 btn-show"
                                   data-id="{{ $return->id }}" title="View">
                                    <i class="bi bi-eye"></i>
                                </a>
                                <a href="{{ route('return_book.edit', $return->id) }}"
                                   class="btn btn-warning btn-sm me-1" title="Edit">
                                    <i class="bi bi-pencil-square"></i>
                                </a>
                                <form action="{{ route('return_book.destroy', $return->id) }}"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <table class="table table-bordered">
                            ...
                            <tr><th>Borrow Date</th><td id="modalBorrow"></td></tr>
                            <tr><th>Due Date</th><td id="modalDue"></td></tr>
                            <tr><th>Return Date</th><td id="modalReturn"></td></tr>
                            <tr><th>Fine</th><td id="modalFine"></td></tr>
...
@section('other_script')
    ...
    <script>
        $(document).on('click', '.btn-show', function () {
            ...
            $.get(`/admin/return_book/${id}`, function (r) {
                $('#modalCode').text(r.code_member ?? '-');
                $('#modalUser').text(r.user_name ?? '-');
                $('#modalBook').text(r.book_title ?? '-');
                $('#modalBorrow').text(r.borrow_date ?? '-');
                $('#modalDue').text(r.due_date ?? '-');
                $('#modalReturn').text(r.return_date ?? '-');
                $('#modalFine').text('$' + (r.fine ?? '0.00'));
                $('#modalStatus').html(`<span class="badge bg-${r.status_badge}">${r.status_label}</span>`);
