...
@section('content')
    <main>
        ...
        <div class="recent-activity">
            <h2 class="activity-title">Edit Book
            ...
            <form action="{{ route('manage_book.update', $book->id) }}" method="POST"
                ...
                <div class="mb-3 col-md-4">
                    ...
                    <select name="category" class="form-select @error('category') is-invalid @enderror">
                        ...
                        <option value="reference book" {{ old('category', $book->category) == 'reference book' ? 'selected' : '' }}>Reference Book</option>
                        <option value="research"       {{ old('category', $book->category) == 'research'       ? 'selected' : '' }}>Research</option>
                        <option value="thesis"         {{ old('category', $book->category) == 'thesis'         ? 'selected' : '' }}>Thesis</option>
                    </select>
                    @error('category')
                        <span class="text-danger">* {{ $message }}</span>
                    @enderror
                ...
                <div class="mb-3 col-md-4">
                    ...
                    <select name="language" class="form-select @error('language') is-invalid @enderror">
                        <option value="khmer"   {{ old('language', $book->language) == 'khmer'   ? 'selected' : '' }}>Khmer</option>
                    ...
                    <label class="form-label upload-box image-box">
                        ...
                        <div class="upload-content">
                            <img id="imagePreview"
                                 src="{{ $book->image_url }}"
                                 alt="Book Cover"
                                 style="display:block;max-height:140px;object-fit:contain;">
                        </div>
                    </label>
                    <small class="text-muted">Upload new image to replace current cover (max 2MB)</small>
                    @error('image')
                        <span class="text-danger">* {{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="mb-3 col-md-5 upload-card">
                    <label class="form-label upload-box file-box">
                        ...
                        <div class="upload-content">
                            <i class="bi bi-filetype-pdf" style="{{ $book->file ? 'color:#dc3545' : '' }}"></i>
                            <p id="pdfName">
                                @if($book->file)
                                    PDF Uploaded &nbsp;<small class="text-success">(click to replace)</small>
                                @else
                                    Upload PDF <small class="text-muted">(optional, max 20MB)</small>
                                @endif
                            </p>
                        </div>
                    </label>
                    @if($book->file)
                        <a href="{{ route('manage_book.download', $book->id) }}" class="btn btn-sm btn-outline-danger mt-1" target="_blank">
