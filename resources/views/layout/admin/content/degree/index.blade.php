    ...
    <main>
        ...
        <div class="info-data">
            ...
            <div class="card-dashboard" data-aos="fade-up">
                <div class="head">
                    <h2>Degrees Added Today</h2>
                    <span class="material-symbols-rounded icon icon-people">developer_guide</span>
                </div>
                <p class="number-people">{{ $degreesToday }}</p>
            </div>

            <div class="card-dashboard" data-aos="fade-up">
                <div class="head">
                    <h2>Total Degrees</h2>
                    <span class="material-symbols-rounded icon icon-member">school</span>
                </div>
                <p class="totle-member">{{ $totalDegrees }}</p>
            </div>
        ...
        <div class="recent-activity">
            <h5 class="activity-title">Degree Programs

                <a href="{{ route('degree.create') }}" class="btn btn-primary float-end mb-2">
            ...
            <table id="example" class="table table-striped table-hover text-center table-bordered nowrap">
                ...
                <tbody class="table-light">
                    ...
                    @foreach ($degrees as $degree)
                        @php
                            $levelColor = match($degree->degree_level) {
                                'bachelor'  => 'success',
                                'associate' => 'primary',
                                'technical' => 'warning',
                                default     => 'secondary',
                        ...
                        @endphp
                        <tr>
                            <td class="text-start">{{ $degree->majors_formatted }}</td>
                            <td>
                                <span class="badge bg-{{ $levelColor }}">
                                    {{ Str::ucfirst($degree->degree_level) }}
                                </span>
                            </td>
                            <td>{{ Str::title($degree->duration_years) }}</td>
                            <td>{{ Str::title($degree->study_time) }}</td>
                            <td>{{ $degree->generation_text }}</td>
                            <td>
                                <a class="btn btn-success btn-sm me-1 btn-show"
                                   data-id="{{ $degree->id }}" title="View">
                                    <i class="bi bi-eye"></i>
                                </a>
                                ...
                                <form action="{{ route('degree.destroy', $degree->id) }}"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Degree Detail</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body">
                        <table class="table table-bordered">
                            <tr><th>Major</th><td id="modalMajor"></td></tr>
                            <tr><th>Degree Level</th><td id="modalLevel"></td></tr>
                            <tr><th>Duration</th><td id="modalDuration"></td></tr>
                            <tr><th>Study Time</th><td id="modalStudyTime"></td></tr>
                            <tr><th>Generation</th><td id="modalGeneration"></td></tr>
                        </table>
                    </div>
                </div>
            </div>
...
@section('other_script')
    ...
    <script>
        $(document).on('click', '.btn-show', function () {
            ...
            $.get(`/admin/degree/${id}`, function (d) {
                $('#modalMajor').text(d.majors_formatted ?? d.majors ?? '-');
                $('#modalLevel').text(d.degree_level
                    ? d.degree_level.charAt(0).toUpperCase() + d.degree_level.slice(1)
                    : '-');
                $('#modalDuration').text(d.duration_years ?? '-');
                $('#modalStudyTime').text(d.study_time ?? '-');
                $('#modalGeneration').text(d.generation_text ?? d.generation ?? '-');
